...
class SyncComparator:
            ...
            if name in remote_dict:
                ...
                if update_existing:
                    changes = self._compare_interface_fields(
                        local_data, remote_dict[name], compare_fields
                    )
                    if changes:
    ...
    def _compare_interface_fields(
        ...
        for field_name in fields:
            local_val = self._get_local_field(local, field_name)
    ...
    def _get_local_field(self, data: Dict[str, Any], field_name: str) -> Any:
