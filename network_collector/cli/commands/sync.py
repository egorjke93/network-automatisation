            ...
            if interfaces:
                ...
                if ip_entries:
                    ...
                    if stats.get("details"):
                        ...
                        # Копируем primary_ip если был установлен
                        if stats["details"].get("primary_ip"):
                            all_details["ip_addresses"]["primary_ip"] = stats["details"]["primary_ip"]
...
def _print_changes_details(all_details: dict, args) -> None:
    ...
    # Выводим primary_ip если был установлен
    ip_details = all_details.get("ip_addresses", {})
    primary_ip = ip_details.get("primary_ip") if isinstance(ip_details, dict) else None
    if primary_ip:
        print(f"\nPRIMARY IP:")
        print(f"  ★ {primary_ip.get('device', '')}: {primary_ip.get('address', '')}")
