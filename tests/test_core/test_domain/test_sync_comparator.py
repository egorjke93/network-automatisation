...
class TestInterfaceDescriptionClearing:
    """
    Тесты для очистки description интерфейса.

    Bug fix: пустой description должен обновлять интерфейс в NetBox,
    а не игнорироваться.
    """
    ...
    def test_empty_description_clears_existing(self, comparator):
        """Пустой description должен очистить существующее описание."""
        local = [{"name": "Gi0/1", "description": ""}]
        remote = Mock()
        remote.name = "Gi0/1"
        remote.description = "Old description to be cleared"
        remote.enabled = True
        remote.mode = None
        ...
        assert len(diff.to_update) == 1
        changes = diff.to_update[0].changes
        desc_change = next((c for c in changes if c.field == "description"), None)
        assert desc_change is not None, "Description change should be detected"
        assert desc_change.old_value == "Old description to be cleared"
        assert desc_change.new_value == ""

    def test_description_not_in_data_skips_comparison(self, comparator):
        """Если description отсутствует в данных, сравнение пропускается."""
        local = [{"name": "Gi0/1"}]  # Нет description в данных
        remote = Mock()
        remote.name = "Gi0/1"
        remote.description = "Keep this description"
        remote.enabled = True
        remote.mode = None
        ...
        # Не должно быть изменений, т.к. description не в данных
        if diff.to_update:
            changes = diff.to_update[0].changes
            desc_changes = [c for c in changes if c.field == "description"]
            assert len(desc_changes) == 0, "No description change if field not in data"
        else:
            assert len(diff.to_skip) == 1

    def test_same_description_no_change(self, comparator):
        """Одинаковый description не вызывает изменения."""
        local = [{"name": "Gi0/1", "description": "Same text"}]
        remote = Mock()
        remote.name = "Gi0/1"
        remote.description = "Same text"
        remote.enabled = True
        remote.mode = None
    ...
    def test_both_empty_description_no_change(self, comparator):
        """Оба пустых description не вызывают изменения."""
        local = [{"name": "Gi0/1", "description": ""}]
        remote = Mock()
        remote.name = "Gi0/1"
        remote.description = ""
