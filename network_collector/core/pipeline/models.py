...
AVAILABLE_SYNC_TARGETS = {
    "devices", "interfaces", "cables", "inventory", "vlans"
}

# Зависимости: target -> requires
SYNC_DEPENDENCIES = {
...
class Pipeline:
    ...
    def validate(self) -> List[str]:
        ...
        # Проверяем что sync шаги имеют соответствующие collect шаги
        collected_targets = set()
        for step in self.steps:
            if step.type == StepType.COLLECT and step.enabled:
                collected_targets.add(step.target)
            elif step.type == StepType.SYNC and step.enabled:
                # Sync требует collect для того же target
                if step.target not in collected_targets:
                    # Проверяем есть ли collect для этого target ДО sync
                    has_collect = any(
                        s.target == step.target and s.type == StepType.COLLECT and s.enabled
                        for s in self.steps[:self.steps.index(step)]
                    )
                    if not has_collect:
                        errors.append(
                            f"Sync '{step.target}' requires collect '{step.target}' first"
                        )
