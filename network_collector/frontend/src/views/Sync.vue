      ...
      <div class="stats-grid">
        <div class="stat-card" v-if="hasStats(result.devices)">
          ...
          <div class="stat-values">
            <span class="created" v-if="result.devices.created">+{{ result.devices.created }}</span>
            <span class="updated" v-if="result.devices.updated">~{{ result.devices.updated }}</span>
            <span class="deleted" v-if="result.devices.deleted">-{{ result.devices.deleted }}</span>
            <span class="skipped" v-if="result.devices.skipped">={{ result.devices.skipped }}</span>
          </div>
        </div>
        <div class="stat-card" v-if="hasStats(result.interfaces)">
          <h4>Interfaces</h4>
          <div class="stat-values">
            <span class="created" v-if="result.interfaces.created">+{{ result.interfaces.created }}</span>
            <span class="updated" v-if="result.interfaces.updated">~{{ result.interfaces.updated }}</span>
            <span class="deleted" v-if="result.interfaces.deleted">-{{ result.interfaces.deleted }}</span>
            <span class="skipped" v-if="result.interfaces.skipped">={{ result.interfaces.skipped }}</span>
          </div>
        </div>
        <div class="stat-card" v-if="hasStats(result.ip_addresses)">
          <h4>IP Addresses</h4>
          <div class="stat-values">
            <span class="created" v-if="result.ip_addresses.created">+{{ result.ip_addresses.created }}</span>
            <span class="updated" v-if="result.ip_addresses.updated">~{{ result.ip_addresses.updated }}</span>
            <span class="deleted" v-if="result.ip_addresses.deleted">-{{ result.ip_addresses.deleted }}</span>
            <span class="skipped" v-if="result.ip_addresses.skipped">={{ result.ip_addresses.skipped }}</span>
          </div>
        </div>
        <div class="stat-card" v-if="hasStats(result.vlans)">
          <h4>VLANs</h4>
          <div class="stat-values">
            <span class="created" v-if="result.vlans.created">+{{ result.vlans.created }}</span>
            <span class="skipped" v-if="result.vlans.skipped">={{ result.vlans.skipped }}</span>
          </div>
        </div>
        <div class="stat-card" v-if="hasStats(result.inventory)">
          <h4>Inventory</h4>
          <div class="stat-values">
            <span class="created" v-if="result.inventory.created">+{{ result.inventory.created }}</span>
            <span class="updated" v-if="result.inventory.updated">~{{ result.inventory.updated }}</span>
            <span class="skipped" v-if="result.inventory.skipped">={{ result.inventory.skipped }}</span>
          </div>
        </div>
        <div class="stat-card" v-if="hasStats(result.cables)">
          <h4>Cables</h4>
          <div class="stat-values">
            <span class="created" v-if="result.cables.created">+{{ result.cables.created }}</span>
            <span class="deleted" v-if="result.cables.deleted">-{{ result.cables.deleted }}</span>
            <span class="skipped" v-if="result.cables.skipped">={{ result.cables.skipped }}</span>
            <span class="failed" v-if="result.cables.failed">x{{ result.cables.failed }}</span>
          </div>
        </div>
      </div>
