<template>
  <div class="page">
    <h1>Inventory</h1>
    <p class="subtitle">Сбор модулей, SFP, PSU</p>
    ...
    <div v-if="result">
      <div class="result-header">
        <h3>Inventory ({{ result.total }} items)</h3>
...
export default {
  ...
  methods: {
    async loadColumns() {
      try {
        const res = await axios.get('/api/fields/inventory')
    ...
    async collect() {
      ...
      try {
        const res = await axios.post('/api/inventory/collect', {
    ...
    onTaskComplete(task) {
      ...
      if (task.status === 'completed' && task.result?.data) {
        this.result = {
          ...
          items: task.result.data,  // Данные уже в правильном формате
        }
