<template>
  <div class="page">
    <h1>Interfaces</h1>
    <p class="subtitle">Сбор интерфейсов с IP, MAC, статусом</p>
    ...
    <div v-if="result">
      ...
      <DataTable
        v-if="result.interfaces && result.interfaces.length"
        :data="result.interfaces"
        :columns="columns"
        export-filename="interfaces"
      >
...
export default {
  ...
  methods: {
    async loadColumns() {
      try {
        const res = await axios.get('/api/fields/interfaces')
    ...
    async collect() {
      ...
      try {
        const res = await axios.post('/api/interfaces/collect', {
    ...
    onTaskComplete(task) {
      ...
      if (task.status === 'completed' && task.result?.data) {
        this.result = {
          ...
          interfaces: task.result.data,  // Данные уже в правильном формате
        }
...
.status-badge.up { background: #d4edda; color: #155724; }
