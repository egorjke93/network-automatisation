...
class InventorySyncMixin:
    ...
    def sync_inventory(
        ...
        if create_batch and not self.dry_run:
            ...
            try:
                self.client.bulk_create_inventory_items(batch_data)
                for _, name, truncate_note in create_batch:
                    logger.info(f"Создан inventory: {name}{truncate_note}")
                    stats["created"] += 1
                    details["create"].append({"name": name})
            except Exception as e:
                logger.warning(f"Batch create inventory не удался ({e}), fallback на поштучное создание")
                # Fallback: создаём по одному
                for data, name, truncate_note in create_batch:
                    try:
                        ...
                        logger.error(f"Ошибка валидации inventory {name}: {format_error_for_log(exc)}")
                        stats["failed"] += 1
                    except NetBoxError as exc:
                        logger.error(f"Ошибка NetBox создания inventory {name}: {format_error_for_log(exc)}")
                        stats["failed"] += 1
                    except Exception as exc:
                        logger.error(f"Неизвестная ошибка создания inventory {name}: {exc}")
                        stats["failed"] += 1

        # === BATCH UPDATE ===
        if update_batch and not self.dry_run:
            # Подготавливаем данные для bulk update (каждый dict содержит 'id')
            batch_data = []
            ...
            try:
                ...
                for existing_id, updates, name, changes, truncate_note in update_batch:
                    try:
                        ...
                        logger.error(f"Ошибка NetBox обновления {name}: {format_error_for_log(exc)}")
                        stats["failed"] += 1
                    except Exception as exc:
                        logger.error(f"Неизвестная ошибка обновления {name}: {exc}")
                        stats["failed"] += 1
        ...
        if cleanup:
            ...
            if delete_ids and not self.dry_run:
                try:
                    self.client.bulk_delete_inventory_items(delete_ids)
                    for nb_name in delete_names:
                        logger.info(f"Удалён inventory: {nb_name}")
                        stats["deleted"] += 1
                        details["delete"].append({"name": nb_name})
                except Exception as e:
                    logger.warning(f"Batch delete inventory не удался ({e}), fallback на поштучное удаление")
                    for nb_name, nb_item in existing_items.items():
                        if nb_name not in processed_names and nb_name in delete_names:
                            try:
                                ...
                                logger.error(f"Ошибка удаления inventory {nb_name}: {exc}")
                                stats["failed"] += 1
